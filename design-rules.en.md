...
- **As a paste-into-any-chat** → paste this file (or its key sections) into ChatGPT / Claude / Gemini as a custom instruction, or attach it as a knowledge file. Prefer the condensed **[`core.md`](core.md)** for a one-page inject.
...
- **One focal point per graphic: the TITLE / the product.** Make it 3–5× the body size.
...
- → **Depth (layouts, font sizing, legibility, logo): `references/hospitality-food-services-playbook.md`**
...
|------|-----------|------|
| **A · Native AI text** | Copy **baked into** the AI render, in-scene, end-to-end. Best text spelling: gpt-image-2 (Codex). Keep strings SHORT (brand + headline + 1 location line); append `CRITICAL: every word spelled PERFECTLY`. | Restaurant/venue/food ads, when the user wants a fully-generated visual. **Default since 2026-08.** |
...
6. QA        — build a contact sheet; check every rule below; fix or redo.
